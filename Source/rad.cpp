...
void CRAD::ReadLights(const char * filename)
{
	...
	while (std::getline(ifs, s))
	{
		...
		else if (s.rfind("noshadow ", 0) == 0)
		{
			...
			m_vSzNoShadowMaterialNames.push_back(strdup(s.c_str() + off));
		...
		else if (s.rfind("forcetextureshadow ", 0) == 0)
		{
			...
			m_vSzForcedTextureShadowsOnModel.push_back(strdup(s.c_str() + off));
		...
		strncpy(sztl, s.c_str() + start_off, len);
		sztl[len] = '\0';
		...
		strncpy(tl.name, sztl, MAX_TEXLIGHT_NAME);
		tl.value = v;
...
int CRAD::LightForString(const char * light, Vector & intensity)
{
	...
	argCnt = sscanf(light, "%lf %lf %lf %lf %lf %lf %lf %lf",
