...
skipspace:
	...
	*token_p = 0;
	...
	else if (!strcasecmp(token, "$definemacro"))
	{
		GetToken(false);
		DefineMacro(token);
		return GetToken(crossline);
	}
	else if (!strcasecmp(token, "$definevariable"))
	{
		GetToken(false);
...
bool CScript::ExpandMacroToken(char *&token_p)
{
	if (script->nummacroparams && *script->script_p == '$')
	{
		...
		*(tp + len) = '\0';
		...
		{
			if (strcasecmp(script->macroparam[index], tp) == 0)
				break;
		}
...
bool CScript::ExpandVariableToken(char *&token_p)
{
	if (*script->script_p == '$')
	{
		...
		*(tp + len) = '\0';
		...
		{
			if (strncmp(m_vDefineVariable[index].param, tp, len) == 0)
				break;
		}
...
bool CScript::AddMacroToStack(char *macroname)
{
	...
	{
		if (strcmp(macrolist[i]->filename, &macroname[1]) == 0)
		{
			break;
		}
	}
